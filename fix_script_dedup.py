...
# Find the first <script> block that contains closeVideoResult (line ~430)
# and the </script> that ends it (line ~722)
script_start = None
script_end = None

for i, line in enumerate(lines):
    if '<script>' in line and script_start is None:
        # check if this block contains closeVideoResult
        for j in range(i, min(i+300, len(lines))):
            if 'function closeVideoResult' in lines[j]:
                script_start = i
                break
        if script_start:
            # find the closing </script>
            for j in range(script_start, min(script_start+400, len(lines))):
                if '</script>' in lines[j]:
                    script_end = j
                    break
            break

print(f"Script block: lines {script_start} to {script_end}")
...
new_script = '''                    <script>
                        ...
                        function vpInit(frames, fps) {
                            ...
                            // Show container first so elements are in DOM
...
if script_start is not None and script_end is not None:
    ...
    print(f"Replaced script block ({script_end - script_start + 1} lines → 1 block)")
